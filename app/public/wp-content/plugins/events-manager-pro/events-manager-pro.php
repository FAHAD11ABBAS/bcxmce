...
Copyright (C) 2025 Pixelite SL
*/

define('EMP_VERSION', '3.7.2.3');
define('EM_MIN_VERSION', '7.2.2');
define('EM_MIN_VERSION_CRITICAL', '6.5.4.1');
define('EMP_SLUG', plugin_basename( __FILE__ ));
define('EMP_DIR', dirname( __FILE__ )); //an absolute path to this directory
define('EMP_DIR_URI', trailingslashit(plugins_url('',__FILE__))); //an absolute path to this directory

// for other plugins backcompat, will eventually depreciate
...
class EM_Pro {
	...
	public static function init(){
		...
		$requirements = new \EM\Requirements_Check( array(
			'title' => 'Events Manager Pro',
			'file' => __FILE__,
			'php' => '7.0',
			'version' => EMP_VERSION,
			'installed' => 'em_pro_version',
			'dependencies' => array(
				'events-manager' => array(
					'name' => 'Events Manager',
					'version' => 'EM_VERSION',
					'minimum' => EM_MIN_VERSION_CRITICAL,
					'recommended' => EM_MIN_VERSION,
...
add_action( 'events_manager_plugin_loaded', 'EM_Pro::init' );
...
add_action('init', 'emp_load_plugin_textdomain');
...
/**
 * Detect if EM was installed, if not leave a warning, since Pro won't load if not triggered by events_manager_plugin_loaded action
 */
function events_manager_pro_loaded () {
	if( !defined('EM_VERSION') ) {
		$install_warning = function(){
			?>
		...
		add_action('admin_notices', $install_warning);
		add_action('network_admin_notices', $install_warning);
	}
}
add_action('plugins_loaded', 'events_manager_pro_loaded' ); // check if EM_VERSION is defined, therefore EM was not loaded
?>