...
class EM_Multiple_Booking extends EM_Booking{
	...
	public static $hide_tickets_bookings;
	...
	public function save ( $mail = true ) {
		static::$hide_tickets_bookings = true;
		if ( empty($this->booking_id) ) {
			$added = true;
			remove_filter('emp_form_show_reg_fields', '__return_false');
		}
		//save the booking
		$return = parent::save( $mail );
		static::$hide_tickets_bookings = false;
		if ( $added ) {
	...
	function save_bookings(){
		...
		global $wpdb;
		$result = true;
		$ignore_keys = apply_filters('em_multiple_booking_ignore_meta_keys', ['waitlist', 'waitlist_booking', 'booking'], $this);
		foreach($this->get_bookings() as $EM_Booking){
			...
			foreach( $this->booking_meta as $meta_key => $meta_value ){
				if( !in_array( $meta_key, $ignore_keys ) ) {
					if ( empty( $EM_Booking->booking_meta[ $meta_key ] ) ) {
						$EM_Booking->booking_meta[ $meta_key ] = $meta_value;
					} else {
						// recursively merge, with booking itself taking precedence
						$rescursive_merge = function( $base_array, $overriding_array, $fn ){
							foreach( $base_array as $base_k => $base_v ){
								if ( is_array( $base_v ) && !empty($overriding_array[$base_k]) ) {
									$fn( $base_v, $overriding_array[$base_k], $fn );
								} elseif ( !empty($overriding_array[$base_k]) ) {
									$base_array[$base_k] = $overriding_array[$base_k]; // overwrite into base array
								}
							}
							return $base_array;
						};
						$EM_Booking->booking_meta[ $meta_key ] = $rescursive_merge( $this->booking_meta[ $meta_key ], $EM_Booking->booking_meta[ $meta_key ], $rescursive_merge );
					}
				}
			}
	...
	function update_meta ( $meta_key, $meta_value, $subkey = null ) {
		$result = parent::update_meta( $meta_key, $meta_value );
		//save other sub-bookings
		if ( $meta_key === 'registration' ) {
			foreach ( $this->get_bookings() as $EM_Booking ) {
				/* @var EM_Booking $EM_Booking */
				if ( $this->booking_id != $EM_Booking->booking_id ) {
					//save data
					$EM_Booking->update_meta( $meta_key, $meta_value );
				}
			}
		}
		return $result;
	...
	function update_user_meta ( $meta_key, $meta_value ) {
		parent::update_user_meta( $meta_key, $meta_value );
		//save other sub-bookings if no-user booking
		if ( $this->person_id === 0 ) {
			foreach( $this->get_bookings() as $EM_Booking){ /* @var EM_Booking $EM_Booking */
				if( $this->booking_id != $EM_Booking->booking_id ){
					$EM_Booking->update_user_meta( $meta_key, $meta_value );
				}
			}
		}
	}
	...
	function get_tickets_bookings(){
		if ( static::$hide_tickets_bookings ) return new EM_Tickets_Bookings(); // short-circuit
