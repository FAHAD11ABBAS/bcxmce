...
class EM_Emails {
	...
	public static function init() {
		...
		if( em_get_option('dbem_custom_emails') ){
			include('custom-emails.php');
		}
		if( em_get_option('dbem_email_bookings')){
		...
		foreach ( Archetypes::get_event_types() as $archetype ) {
			Archetypes::set_current( $archetype );
			if( em_get_option('dbem_cron_emails', 1) ) {
				//set up cron for addint to email queue
				if( !wp_next_scheduled('emp_cron_emails_queue') ){
					$todays_time_to_run = strtotime(date('Y-m-d', current_time('timestamp')).' '.  em_get_option('dbem_emp_emails_reminder_time'), current_time('timestamp'));
					$tomorrows_time_to_run = strtotime(date('Y-m-d', current_time('timestamp')+(86400)).' '. em_get_option('dbem_emp_emails_reminder_time'), current_time('timestamp'));
					$time = $todays_time_to_run > current_time('timestamp') ? $todays_time_to_run:$tomorrows_time_to_run;
					$time -= ( em_get_option( 'gmt_offset' ) * HOUR_IN_SECONDS ); //offset time to run at UTC time for WP Cron
					$result = wp_schedule_event( $time,'daily','emp_cron_emails_queue', [$archetype]);
				}
				$scheduled = true;
			}else{
		...
		// archetype-independent email queue processing
	...
	public static function booking_email_ical_attachments( $msg, $EM_Booking ){
		...
		if( get_class($EM_Booking) == 'EM_Multiple_Booking' ){
			if( !em_get_option('dbem_multiple_bookings_ical_attachments') ){
				return $msg;
			}
			...
			if( !em_get_option('dbem_bookings_ical_attachments') ){
				return $msg;
			}
	 ...
	 */
	public static function queue_emails( $archetype = null ) {
	    global $wpdb;
	    //For each event x days on
		Archetypes::set_current( $archetype );
	    $days = em_get_option('dbem_emp_emails_reminder_days',1);
		...
		$output_type = em_get_option('dbem_smtp_html') ? 'html':'email';
	    foreach( EM_Events::get( [ 'scope'=>$scope,'private'=>1,'blog'=>get_current_blog_id(), 'event_archetype' => $archetype ] ) as $EM_Event ){
	        ...
	        foreach( $EM_Event->get_bookings()->get_bookings()->bookings as $EM_Booking ){ //get confirmed bookings
	            ...
	            if( is_email($EM_Booking->get_person()->user_email) ){
	                ...
	                if( empty($subject_format) ){
		                $subject_format = em_get_option('dbem_emp_emails_reminder_subject');
		                $message_format = em_get_option('dbem_emp_emails_reminder_body');
	                }
	        ...
	        if(count($emails) > 0){
	            ...
	            if( em_get_option('dbem_emp_emails_reminder_ical') ){
		            //create invite ical
