...
class Email_Bookings {
	...
	public static function email_page(){
		...
		$subject = !empty($_POST['subject']) ? wp_unslash($_POST['subject']) : em_get_option('dbem_email_bookings_default_subject');
		
		if ( (defined('EM_UNFILTERED_HTML') && constant('EM_UNFILTERED_HTML')) || current_user_can('unfiltered_html') ) {
			$message = !empty($_POST['message']) ? wp_unslash($_POST['message']) : em_get_option('dbem_email_bookings_default_body');
		} else {
			$message = !empty($_POST['message']) ? wp_kses_post( wp_unslash($_POST['message']) ) : em_get_option('dbem_email_bookings_default_body');
		}
		?>
		<div class='wrap'>
			<?php if( is_admin() ): ?>
			...
			<form action="" method="post" enctype="multipart/form-data" class="em-email-event-bookings">
				...
				<p><?php echo sprintf( esc_html__('The email will be sent from %s, but when the user the following email will be used instead.', 'events-manager-pro'), '<code>' . em_get_option('dbem_mail_sender_address') . '</code>' ); ?></p>
				<p>
