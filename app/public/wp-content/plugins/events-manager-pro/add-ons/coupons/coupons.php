...
class EM_Coupons extends EM_Object {
	...
	/**
	 * Temporary cache of coupons that get re-saved over and over for recurring events.
	 * @var array
	 */
	static public $saving_recurrences_coupons = [];
	...
	public static function init(){
		...
		if( get_option('dbem_multiple_bookings') ){ //multiple bookings mode
			...
			add_action('em_cart_footer', [ static::class, 'em_cart_footer' ]);
			//hook into booking submission to add discount and coupon info
			add_filter('em_multiple_booking_validate', [ static::class, 'em_booking_validate' ], 10, 2);
			add_filter('em_multiple_booking_save', [ static::class, 'em_booking_save' ], 10, 2);
			//ajax to apply coupon
			add_action('wp_ajax_em_coupon_apply',[ static::class, 'cart_coupon_apply_ajax' ]);
			add_action('wp_ajax_nopriv_em_coupon_apply',[ static::class, 'cart_coupon_apply_ajax' ]);
			//AJAX fallback to apply coupon
			if( !empty($_REQUEST['action']) && $_REQUEST['action'] == 'em_coupon_apply' && !defined('DOING_AJAX') ){
				add_action('wp_loaded', [ static::class, 'cart_coupon_apply_fallback' ], 100);
			}
			//manual bookings - add coupons for single event manual bookings
			if( !empty($_REQUEST['action']) && $_REQUEST['action'] == 'manual_booking' ){
				add_action('em_booking_form_summary_footer', [ static::class, 'em_booking_form' ],1,2);
				add_action('em_booking_form_confirm_header', [ static::class, 'em_booking_form' ],1,2);
			}
			...
			add_filter('em_event_get_post_meta',[ static::class, 'em_event_get_post_meta' ],10,2);
			add_filter('em_event_save_meta',[ static::class, 'em_event_save_meta' ],10,2);
			add_filter('em_recurrence_set_save_recurrences_pre',[ static::class, 'em_recurrence_set_save_recurrences_pre' ],10,1);
			add_filter('em_recurrence_set_save_recurrences',[ static::class, 'em_recurrence_set_save_recurrences' ],10,2);
			add_filter('em_event_delete_meta',[ static::class, 'em_event_delete_meta' ],10,2);
			//hook into booking submission to add discount and coupon info
			add_filter('em_booking_get_post', [ static::class, 'em_booking_get_post' ], 10, 2);
			add_filter('em_booking_validate', [ static::class, 'em_booking_validate' ], 10, 2);
			add_filter('em_booking_save', [ static::class, 'em_booking_save' ], 10, 2);
		...
		// load JS
		add_action('em_booking_form_assets', [ static::class, 'booking_form_js' ],10,3);
		add_action('em_enqueue_assets', [ static::class, 'cart_js' ],10,3);
		//add ajax response for coupon code queries
		add_action('wp_ajax_em_coupon_check',[ static::class, 'coupon_check_ajax' ]);
		add_action('wp_ajax_nopriv_em_coupon_check',[ static::class, 'coupon_check_ajax' ]);
		//deal with bookings that have coupons when they get deleted or cancelled
		add_filter('em_booking_delete', [ static::class, 'em_booking_delete' ], 10, 2);
		add_filter('em_booking_set_status', [ static::class, 'em_booking_set_status' ], 10, 2);
		add_filter('em_bookings_delete', [ static::class, 'em_bookings_delete' ], 10, 3);
        //show available coupons on event booking admin area
		add_action('em_events_admin_bookings_footer',[ static::class, 'admin_meta_box' ],20,1);
		//placeholders
		add_filter('em_booking_output_placeholder',[ static::class, 'placeholders' ],1,3); //for email
		//hook into price calculations
		add_filter('em_booking_get_price_adjustments', [ static::class, 'em_booking_get_price_adjustments' ], 10, 3);
		//add coupon info to CSV
		add_action('em_bookings_table_cols_template', [ static::class, 'em_bookings_table_cols_template' ], 10, 2);
		add_action('em_bookings_table_cols_template_groups', [ static::class, 'em_bookings_table_cols_template_groups' ],10,1);
		add_filter('em_bookings_table_rows_col_coupon', [ static::class, 'em_bookings_table_rows_col_coupon' ], 10, 3);
	}
	...
	/**
	 * Remove the individual save meta hooks to make saving more efficient
	 * @param $result
	 * @param $Recurrence_Set
	 *
	 * @return void
	 */
	public static function em_recurrence_set_save_recurrences_pre( $Recurrence_Set ){
		remove_filter('em_event_save_meta',[ static::class, 'em_event_save_meta' ],10);
	}
	...
	/**
	 * @param $result
	 * @param \EM\Recurrences\Recurrence_Set $Recurrence_Set
	 *
	 * @return mixed
	 */
	public static function em_recurrence_set_save_recurrences( $result, $Recurrence_Set ){
		global $wpdb;
		if( $result ){
			// get all updated and added event ids
			$event_ids = array_keys($Recurrence_Set->save_status['events']['updated']) + array_keys($Recurrence_Set->save_status['events']['added']);
			...
			foreach( self::event_get_coupons( $Recurrence_Set->get_event() ) as $EM_Coupon ){
				$insert_templates[] = "( %d, 'event-coupon', ". $wpdb->prepare("%d )", array($EM_Coupon->coupon_id));
			}
			// insert via templates in one insert statement
		...
		// readd filter and return result
		if ( !has_filter( 'em_event_save_meta', [ static::class, 'em_event_save_meta' ] ) ) {
			add_filter( 'em_event_save_meta', [ static::class, 'em_event_save_meta' ], 10, 2 );
		}
		return $result;
	}
	...
	public static function em_event_delete_meta($result, $EM_Event){
		...
		global $wpdb;
		if( $result ){
			$result_coupons = $wpdb->query( "DELETE FROM ".EM_META_TABLE." WHERE meta_key='event-coupon' AND object_id=". $EM_Event->get_event_id() );
		}
			...
			<div class="em-booking-form-section-coupons em-booking-section <?php if ( $EM_Booking->get_spaces() == 0 ) echo 'hidden'; ?>">
				<div class="input-type em-bookings-form-coupon">
					...
					<div class="em-coupon-code-fields">
						...
						<button type="button" class="em-coupon-code-button" data-text-apply="<?php esc_attr_e('Apply') ?>" data-text-remove="<?php esc_attr_e('Remove'); ?>">
	 ...
	 * @return array
	 */
	public static function booking_form_js( $assets ) {
		$url = trailingslashit( plugins_url( '', __FILE__ ) );
		$js = EM_Scripts_and_Styles::get_minified_extension_js().'.js'.'?v='.EMP_VERSION;
		$css = EM_Scripts_and_Styles::get_minified_extension_css().'.css'.'?v='.EMP_VERSION;
		$assets['js']['em-coupons'] = [ 'url' => $url . 'coupons' . $js, 'requires' => 'em-bookings' ];
		$assets['css']['em-coupons'] = $url . 'coupons'.$css;
		return $assets;
	}

	public static function cart_js( $assets ) {
		$url = trailingslashit( plugins_url( '', __FILE__ ) );
		$js = EM_Scripts_and_Styles::get_minified_extension_js().'.js'.'?v='.EMP_VERSION;
		$css = EM_Scripts_and_Styles::get_minified_extension_css().'.css'.'?v='.EMP_VERSION;
		$assets['.em-cart-coupons-form'] = [
			'js' => [
				'em-coupons-cart' => ['url' => $url . 'coupons-cart'. $js, 'event' => 'em_timepicker_ready'],
			],
			'css' => [
				'em-coupons' => $url . 'coupons'.$css,
			],
		];
		return $assets;
	}
	...
	public static function get_default_search($array_or_defaults=array(), $array = array()){
		...
		if ( !empty($array['event']) && is_string($array['event']) && !is_numeric($array['event']) ){
			$event_id = explode(':', $array['event']);
			$array['event'] = $event_id[0];
		}
