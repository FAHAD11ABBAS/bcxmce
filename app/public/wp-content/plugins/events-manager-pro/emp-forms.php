<?php
use EM\Uploads\Uploader, EM\Forms\Uploads, EM\Utils;
...
class EM_Forms {
	...
	public static function init(){
		...
		include('add-ons/emp-form-uploads.php');
	}
...
class EM_Form extends EM_Object {
	...
	public $_request_path = [];
	protected $custom_user_fields = array();
	public $form_required_error = '';
	static $validate;
	/**
	 * @var string  If set, this filename pattern will be used for uploads rather than the supplied filename of this field, {field_id} would be replaced by the actual field id of the form field.
	 */
	public $uploads_filename;
	/**
	 * @var bool    If set to false, the file will be uploaded directly to $this->uploads_folder path rather than a subfolder, if set to true, the field ID will be used as a subfolder. If set to a string this subfolder path will be used.
	 */
	public $uploads_subfolder = true;
	/**
	 * @var string  A unique ID for locating uploaded files via the upload endpoint
	 */
	public $uploads_endpoint_uuid;
	/**
	 * @var int
	 */
	public $uploads_user_id;
	...
	function get_post( $validate = true ){
		...
		$REQUEST = Utils::_request( $this->_request_path );
		foreach($this->form_fields as $field){
			// if this is a custom user field, get the real field type
			if( array_key_exists($field['type'], $this->custom_user_fields) && array_key_exists($field['fieldid'], $custom_user_fields) ){
				$field_type = $custom_user_fields[$field['fieldid']]['type'];
			} else {
				$field_type = $field['type'];
			}
			// now check field type
			$fieldid = $field['fieldid'];
			if ( $field_type !== 'upload' ) { // ignore uploads entirely, they should only be added upon validation
				$value = '';
				if( !isset($REQUEST[$fieldid]) ){ //for things like checkboxes when editing
				    $REQUEST[$fieldid] = '';
				    if($field['type'] == 'checkbox') $REQUEST[$fieldid] = '0'; //force save a 0 rather than a blank so we can check it
				}
				if( !is_array($REQUEST[$fieldid])){
					$this->field_values[$fieldid] = wp_kses_data(stripslashes($REQUEST[$fieldid]));
				}elseif( is_array($REQUEST[$fieldid])){
				    $array = array();
				    foreach( $REQUEST[$fieldid] as $key => $array_value ){
				...
				if( in_array( $field['type'], array('date','time')) ){
					if( !empty($REQUEST[$fieldid]['start']) ){
						$this->field_values[$fieldid] = wp_kses_data($REQUEST[$fieldid]['start']);
						if( $field['options_'.$field['type'].'_range'] && !empty($REQUEST[$fieldid]['end']) ){
							$this->field_values[$fieldid] .= ','. wp_kses_data($REQUEST[$fieldid]['end']);
						}
					}else{
				...
				// add upload data to a specific post variable
				if( empty($this->field_values[$fieldid]) ) $this->field_values[$fieldid] = [];
				$this->field_values[$fieldid]['post'] = [];
				foreach ( ['', '--names', '--deleted'] as $subkey ) {
					if ( !empty($REQUEST[$fieldid . $subkey]) ) {
						$this->field_values[$fieldid]['post'][$fieldid . $subkey] = $REQUEST[$fieldid . $subkey];
					}
				}
			}
		}
	...
	function get_formatted_value( $field, $field_value, $format = 'html' ){
		...
		switch( $field['type'] ){
			...
			case 'country':
				if( $field_value != 'n/a' ){
					...
					if( is_scalar($field_value) && !empty($countries[$field_value]) ) $field_value = $countries[$field_value];
				}
				break;
			case 'tel':
				if( $field_value != 'n/a' ){
					...
					if( EM\Phone::is_enabled() ) {
						...
						$number = EM\Phone::parse( $field_value );
						if( $number ) {
							$field_value = $number;
						} else {
							$field_value = '<span class="em-icon em-icon-warning em-tooltip" aria-label="'. esc_attr__('Invalid Number', 'events-manager') .'"></span> ' . $field_value;
						}
					}
				}
				break;
			case 'upload':
				// create a link to file with a file icon
				if ( !empty($field_value['files']) && !empty($field) ) {
					//$field_value = '<a href="'. esc_attr($field_value['url']) .'" target="_blank">'. end( $url_parts ) .'</a>';
					ob_start();
					if ( $format == 'email' || $format == 'text' ) {
						$urls = [];
						foreach( $field_value['files'] as $uuid => $file ) {
							$urls[] = Uploads::get_file_url( $uuid, $field, $field_value );
						}
						echo implode(', ', $urls);
					} elseif ( $format == 'table' ) {
						$urls = [];
						foreach( $field_value['files'] as $uuid => $file ) {
							$urls[] = '<a href="'. Uploads::get_file_url( $uuid, $field, $field_value ) .'">'. $file['name'] . '</a>';
						}
						echo '<ul class="em-form-upload-field-table"><li>' . implode('</li><li>', $urls) . '</li></ul>';
					} else {
						Uploads::output_field_input( $this, $field, $field_value, [ 'disabled' => true ] );
					}
					$field_value = ob_get_clean();
				} elseif ( !empty($field_value['post']) && !empty($field) ) {
					if ( $format === 'html' ) {
						ob_start();
						//$field['name'] = $field_value['post']; // so $_REQUEST is obtained properly
						Uploads::output_field_input( $this, $field, $field_value, ['disabled' => true], $field_value['post'] );
						$field_value = ob_get_clean();
					} else {
						$field_value = '';
					}
			...
			default:
			    if( is_array($field_value) ){
					if ( count(array_filter($field_value, 'is_array')) === 0 ) {
	...
	function output_field($field, $post=true){
		...
		switch($field['type']){
			...
			case 'upload':
				...
				?>
				<p class="input-group input-<?php echo $field['type']; ?> em-input-<?php echo $field['type']; ?> input-field-<?php echo $field['fieldid'] ?>">
					<label for='<?php echo $field['fieldid'] ?>'>
						...
						<?php if( !empty($field['options_'.$tip_type.'_tip']) ): ?>
							<span class="em-tooltip" data-tippy-placement="right" data-tippy-content="<?php echo esc_attr($field['options_'.$tip_type.'_tip']); ?>">
				...
				if( array_key_exists($field['type'], $this->user_fields) && self::show_reg_fields($field) ){
					if( array_key_exists($field['type'], $this->core_user_fields) ){
						if( $field['type'] == 'user_password' ){
							...
							<p class="input-<?php echo $field['type']; ?> input-user-field">
								<label for='<?php echo $field['fieldid'] ?>'>
									<?php if( !empty($field['options_reg_tip']) ): ?>
										<span class="em-tooltip" data-tippy-placement="right" data-tippy-content="<?php echo esc_attr($field['options_reg_tip']); ?>">
											<?php echo $field['label'] ?> <?php echo $required  ?>
										</span>
									<?php else: ?>
										<?php echo $field['label'] ?> <?php echo $required  ?>
							...
							//registration fields
							if( empty($REQUEST[$field['fieldid']]) && is_user_logged_in() && get_option('dbem_emp_booking_form_reg_show') && !EM_Bookings::$force_registration ){
	...
	function output_field_input($field, $post=true){
		...
		$default_html = '';
		$REQUEST = Utils::_request( $this->_request_path );
		if($post === true && !empty($REQUEST[$field['fieldid']])) {
			$default = is_array($REQUEST[$field['fieldid']]) ? $REQUEST[$field['fieldid']]:esc_attr($REQUEST[$field['fieldid']]);
			$default_html = is_array($REQUEST[$field['fieldid']]) ? $REQUEST[$field['fieldid']] : esc_attr($REQUEST[$field['fieldid']]);
		...
		switch($field['type']){
			...
			case 'tel' :
				?>
				<input type="tel" name="<?php echo $field_name ?>" id="<?php echo $field['fieldid'] ?>" class="input em-phone-intl" value="<?php echo $default; ?>"  />
				<?php
				break;
			case 'textarea':
				$size = 'rows="2" cols="20"';
			...
			case 'country':
				...
				break;
			case 'date':
				if( !empty($REQUEST[$field_name]['start']) && preg_match('/\d{4}-\d{2}-\d{2}/', $REQUEST[$field_name]['start'])) {
					$default = array( $REQUEST[$field_name]['start'] );
					if( !empty($REQUEST[$field_name]['end']) && preg_match('/\d{4}-\d{2}-\d{2}/', $REQUEST[$field_name]['end'])){
						$default[] = $REQUEST[$field_name]['end'];
					}
				}else{
			...
			case 'time':
				...
				if( !empty($REQUEST[$field_name]['start']) && !preg_match('/^([01]\d|2[0-3]):([0-5]\d) ?(AM|PM)?$/', $REQUEST[$field_name]['start']) ) {
					$default = array( $REQUEST[$field_name]['start'] );
					if( !empty($REQUEST[$field_name]['end']) && !preg_match('/^([01]\d|2[0-3]):([0-5]\d) ?(AM|PM)?$/', $REQUEST[$field_name]['end']) ){
						$default[] = $REQUEST[$field_name]['end'];
					}
				}else{
			...
			case 'captcha':
				...
				break;
			case 'upload':
				// TODO - Upload - add limiting settings like extensions permitted
				Uploads::output_field_input( $this, $field, $default );
				break;
			default:
	...
	function validate_field( $field_id, $value ){
		...
		if( is_array($field) ){
			...
			switch($field['type']){
				...
				case 'textarea':
				case 'tel':
					if( $field['type'] !== 'tel' || !EM\Phone::is_enabled() ) {
						//regex
				...
				case 'tel':
					...
					if( trim($value) == '' && !empty($field['required']) ){
						$this->add_error($err);
						$result = false;
					} else if ( trim($value) != '' && !EM\Phone::validate( $value ) ) {
						$err = emp__('Please provide a valid phone number.');
				...
				case 'upload':
					//non-empty match
					$upload_data = !empty($this->field_values[$field_id]['post']) ? $this->field_values[$field_id]['post'] : []; // localize request variables
					try {
						$upload_options = Uploads::get_field_options( $field );
						// if max_files is set, subtract uploaded files from allowed, offsetting files set to delete
						if ( ( !empty($upload_options['required']) || !empty($upload_options['max_files']) ) && !empty($this->field_values[$field_id]['files']) ) {
							$uploaded_files = count($this->field_values[$field_id]['files']);
							// subtract deleting files
							if ( !empty( $upload_data[ $field_id . '--deleted' ] ) && is_array( $upload_data[ $field_id . '--deleted' ] ) ) {
								foreach ( $upload_data[ $field_id . '--deleted' ] as $file_id_to_delete => $delete_it ) {
									// uploader provides a uuid for each upload, we supply that to em-uploader.js for use in retrieving or deleting these files
									if( !empty( $this->field_values[$field_id]['files'][$file_id_to_delete] ) ) {
										$uploaded_files--;
									}
								}
							}
							if ( !empty($upload_options['max_files']) ) {
								$upload_options['max_files'] = $upload_options['max_files'] > $uploaded_files ? $upload_options['max_files'] - $uploaded_files : 0;
							}
							if ( !empty($upload_options['required']) && $uploaded_files > 0 ) {
								$upload_options['required'] = false;
							}
						}
						$field_keypath = !empty($this->_request_path) ? array_merge( $this->_request_path, [$field_id] ) : $field_id; // handle contained request paths
						Uploader::validate( $field_keypath, $upload_options, $upload_data ); // upload the data saved in post, rather than let it look at $_REQUEST
					} catch ( EM_Exception $ex ) {
						if ( $ex->error_code !== 'upload_required' || empty($this->field_values[$field_id]['files']) ) {
							$this->add_error( $ex );
						}
					}
					break;
	...
	function save() {
		...
		foreach( $this->form_fields as $field_id => $field ){
			$result = $this->save_field( $field );
			if( $result === true ) {
				$results[ $field_id ] = $this->field_values[ $field_id ];
			}
		}
		if( count($this->get_errors()) > 0 ){
			return false;
	...
	function save_field( $field ) {
		$field_id = $field['fieldid'];
		if( is_array($field) && $field['type'] === 'upload' ) {
			$upload_data = !empty($this->field_values[$field_id]['post']) ? $this->field_values[$field_id]['post'] : []; // localize request variables
			try {
				$uploads_subdir = Uploads::$uploads_folder;
				if ( $this->uploads_subfolder === true ) {
					$uploads_subdir = trailingslashit($uploads_subdir) . $field_id;
				} elseif ( $this->uploads_subfolder ) {
					$uploads_subdir = trailingslashit($uploads_subdir) . untrailingslashit(ltrim( $this->uploads_subfolder, '/\\' ));
				}
				$file_name = false;
				if ( $this->uploads_filename ) {
					$file_name = str_replace('{field_id}', $field_id, $this->uploads_filename);
				}
				if ( empty( $this->field_values[$field_id] ) ) {
					$this->field_values[$field_id] = ['files' => []];
				}
				// first, check to delete any previous files, in case em-uploader.js removed the file
				if ( !empty( $upload_data[ $field_id . '--deleted' ] ) && is_array( $upload_data[ $field_id . '--deleted' ] ) ) {
					foreach ( $upload_data[ $field_id . '--deleted' ] as $file_id_to_delete => $delete_it ) {
						// uploader provides a uuid for each upload, we supply that to em-uploader.js for use in retrieving or deleting these files
						if( !empty( $this->field_values[$field_id]['files'][$file_id_to_delete] ) ) {
							@unlink( $this->field_values[$field_id]['files'][$file_id_to_delete]['file'] );
							unset( $this->field_values[$field_id]['files'][$file_id_to_delete] );
						}
					}
				}
				// merge previous uploaded files with current one
				$field_keypath = !empty($this->_request_path) ? array_merge( $this->_request_path, [$field_id] ) : $field_id; // handle contained request paths
				$uploads = Uploader::upload( $field_keypath, $uploads_subdir, $file_name );
				if( !empty($uploads) ) {
					if ( !empty($this->field_values[ $field_id ]['files']) ) {
						$this->field_values[ $field_id ]['files'] = array_merge( $this->field_values[ $field_id ]['files'], $uploads );
					} else {
						$this->field_values[ $field_id ]['files'] = $uploads;
					}
					unset($this->field_values[ $field_id ]['post']);
				}
				if ( empty($this->field_values[ $field_id ]['files']) ) {
					$this->field_values[ $field_id ] = null;
				} elseif ( $this->uploads_subfolder ) {
					// add endpoint urls for all the uploads so authorized users can download them
					$this->field_values[ $field_id ]['endpoint'] = $this->uploads_endpoint_uuid;
					$this->field_values[ $field_id ]['subfolder'] = $this->uploads_subfolder;
				}
				return true;
			} catch ( EM_Exception $ex ) {
				$this->add_error( $ex );
			}
		} elseif ( array_key_exists($field['type'], $this->user_fields) && (self::validate_reg_fields($field)) ) {
			//custom field chekcs
			if( array_key_exists($field['type'], $this->custom_user_fields)) {
				//custom field, so just apply
				$saved_field = apply_filters('em_form_save_custom_user_field', null, $field, $this);
				if ( $saved_field ) {
					$this->field_values[ $field_id ] = $saved_field;
					return true;
				}
			}
		}
	...
	function get_fields_map(){
		$map = array (
			...
			'options_upload_error','options_upload_tip','options_upload_allow_multiple','options_upload_max_files','options_upload_max_file_size','options_upload_type','options_upload_extensions','options_upload_image_min_width','options_upload_image_max_width','options_upload_image_min_height','options_upload_image_max_height',
				...
				<ul class="booking-custom-body">
					...
					<li class="booking-custom-item" <?php if( $field_key === 'blank_em_template' ){ echo 'id="booking-custom-item-template"'; }; ?>>
						...
						<div class='bc-col-type bc-col'>
							<select name="type[]" class="booking-form-custom-type">
								...
								<optgroup label="<?php _e('Customizable Fields','em-pro'); ?>">
									...
									<?php endif; ?>
									<option <?php self::input_default('type',$field_values,'select','upload'); ?>>upload</option>
								</optgroup>
								<?php endif; ?>
								<?php if($user_fields): ?>
						...
						<div class='booking-custom-types'>
							...
							<!-- upload -->
							<div class="bct-upload bct-options" style="display:none;">
								<div class="bct-field">
									<div class="bct-label"><?php _e('Tip Text','em-pro'); ?></div>
									<div class="bct-input">
										<input type="text" name="options_upload_tip[]" <?php self::input_default('options_upload_tip',$field_values); ?> />
										<?php if( EM_ML::$is_ml ) $this->editor_ml_fields_trigger('options_upload_tip'); ?>
										<em><?php _e('Will appear next to your field label as a question mark with a popup tip bubble.','em-pro'); ?></em>
									</div>
									<?php if( EM_ML::$is_ml ) $this->editor_ml_fields('options_upload_tip', $field_values); ?>
								</div>
								<div class="bct-field">
									<div class="bct-label"><?php _e('Error Message','em-pro'); ?></div>
									<div class="bct-input">
										<input type="text" name="options_upload_error[]" <?php self::input_default('options_upload_error',$field_values); ?> />
										<?php if( EM_ML::$is_ml ) $this->editor_ml_fields_trigger('options_upload_error'); ?>
										<em>
											<?php _e('If phone number does not validate, this error will be displayed.','em-pro'); ?>
											<br /><?php _e('Default:','em-pro'); echo ' <code>'.sprintf(get_option('dbem_emp_booking_form_error_required'), '[FIELD]').'</code>'; ?>
										</em>
									</div>
									<?php if( EM_ML::$is_ml ) $this->editor_ml_fields('options_upload_error', $field_values); ?>
								</div>
									<div class="bct-field">
										<div class="bct-label"><?php _e('Allow Multiple Uploads','em-pro'); ?></div>
										<div class="bct-input">
											<input type="checkbox" name="options_upload_allow_multiple[]" value="1" <?php self::input_default('options_upload_allow_multiple', $field_values, 'checkbox', Uploader::$default_options['allow_multiple'] ?: ''); ?> />
											<input type="hidden" name="options_upload_allow_multiple[]" <?php self::input_default('options_upload_allow_multiple', $field_values, 'checkbox', Uploader::$default_options['allow_multiple'] ?: ''); ?> />
											<em><?php _e('Enable multiple file uploads.','em-pro'); ?></em>
										</div>
									</div>
									<div class="bct-field">
										<div class="bct-label"><?php _e('Max Files','em-pro'); ?></div>
										<div class="bct-input">
											<input type="number" name="options_upload_max_files[]" <?php self::input_default('options_upload_max_files', $field_values, 'text', Uploader::$default_options['max_files'] ?: ''); ?> />
											<em><?php _e('Maximum number of files allowed.','em-pro'); ?></em>
										</div>
									</div>
									<div class="bct-field">
										<div class="bct-label"><?php _e('File Type','em-pro'); ?></div>
										<div class="bct-input">
											<select name="options_upload_type[]">
												<option value=""><?php _e('Any Type','em-pro'); ?></option>
												<option value="image" <?php echo ((!empty($field_values['options_upload_type']) && $field_values['options_upload_type'] === 'image') || (empty($field_values['options_upload_type']) && ( !empty(Uploader::$default_options['type'][0]) ? Uploader::$default_options['type'][0] === 'image' : false))) ? 'selected="selected"' : ''; ?>><?php _e('Image','em-pro'); ?></option>
												<option value="document" <?php echo ((!empty($field_values['options_upload_type']) && $field_values['options_upload_type'] === 'document') || (empty($field_values['options_upload_type']) && ( !empty(Uploader::$default_options['type'][0]) ? Uploader::$default_options['type'][0] === 'document' : false))) ? 'selected="selected"' : ''; ?>><?php _e('Document','em-pro'); ?></option>
												<option value="spreadsheet" <?php echo ((!empty($field_values['options_upload_type']) && $field_values['options_upload_type'] === 'spreadsheet') || (empty($field_values['options_upload_type']) && ( !empty(Uploader::$default_options['type'][0]) ? Uploader::$default_options['type'][0] === 'spreadsheet' : false))) ? 'selected="selected"' : ''; ?>><?php _e('Spreadsheet','em-pro'); ?></option>
											</select>
											<em><?php _e('Select file type (extensions override this option).','em-pro'); ?></em>
										</div>
									</div>
									<div class="bct-field">
										<div class="bct-label"><?php _e('Allowed Extensions','em-pro'); ?></div>
										<div class="bct-input">
											<input type="text" name="options_upload_extensions[]" <?php self::input_default('options_upload_extensions', $field_values, 'text', !empty(Uploader::$default_options['extensions']) ? implode(', ', Uploader::$default_options['extensions']) : ''); ?> />
											<em><?php _e('Comma‐separated list of allowed file extensions (e.g., jpg, png, pdf).','em-pro'); ?></em>
										</div>
									</div>
									<div class="bct-field">
										<div class="bct-label"><?php _e('Max File Size','em-pro'); ?></div>
										<div class="bct-input">
											<input type="number" name="options_upload_max_file_size[]" <?php self::input_default('options_upload_max_file_size', $field_values, 'text'); ?> />
											<em>
												<?php _e('Maximum file size in bytes.','em-pro'); ?>
												<?php echo Uploader::$default_options['max_file_size'] ? ' Default if blank: <code>' . Uploader::$default_options['max_file_size'] . '</code>' : ''; ?>
											</em>
										</div>
									</div>
									<div class="bct-field">
										<div class="bct-label"><?php _e('Min Image Width','em-pro'); ?></div>
										<div class="bct-input">
											<input type="number" name="options_upload_image_min_width[]" <?php self::input_default('options_upload_image_min_width', $field_values, 'text'); ?> />
											<em>
												<?php _e('Minimum image width in pixels.','em-pro'); ?>
												<?php echo Uploader::$default_options['image_min_width'] ? ' Default if blank: <code>' . Uploader::$default_options['image_min_width'] . '</code>' : ''; ?>
											</em>
										</div>
									</div>
									<div class="bct-field">
										<div class="bct-label"><?php _e('Max Image Width','em-pro'); ?></div>
										<div class="bct-input">
											<input type="number" name="options_upload_image_max_width[]" <?php self::input_default('options_upload_image_max_width', $field_values, 'text'); ?> />
											<em>
												<?php _e('Maximum image width in pixels.','em-pro'); ?>
												<?php echo Uploader::$default_options['image_max_width'] ? ' Default if blank: <code>' . Uploader::$default_options['image_max_width'] . '</code>' : ''; ?>
											</em>
										</div>
									</div>
									<div class="bct-field">
										<div class="bct-label"><?php _e('Min Image Height','em-pro'); ?></div>
										<div class="bct-input">
											<input type="number" name="options_upload_image_min_height[]" <?php self::input_default('options_upload_image_min_height', $field_values, 'text'); ?> />
											<em>
												<?php _e('Minimum image height in pixels.','em-pro'); ?>
												<?php echo Uploader::$default_options['image_min_height'] ? ' Default if blank: <code>' . Uploader::$default_options['image_min_height'] . '</code>' : ''; ?>
											</em>
										</div>
									</div>
									<div class="bct-field">
										<div class="bct-label"><?php _e('Max Image Height','em-pro'); ?></div>
										<div class="bct-input">
											<input type="number" name="options_upload_image_max_height[]" <?php self::input_default('options_upload_image_max_height', $field_values, 'text'); ?> />
											<em>
												<?php _e('Maximum image height in pixels.','em-pro'); ?>
												<?php echo Uploader::$default_options['image_max_height'] ? ' Default if blank: <code>' . Uploader::$default_options['image_max_height'] . '</code>' : ''; ?>
											</em>
										</div>
									</div>


								<?php do_action('emp_forms_editor_upload_options', $this, $field_values); ?>
							</div>
